...
def visualize_coefficients(coefs, names, year):
    posCoefs = coefs.copy()
    negCoefs = coefs.copy()
    ...
    ax.bar(names, posCoefs, align="edge", width=0.5, color="r")
    ax.bar(names, negCoefs, align="edge", width=0.5, color="b")
    ax.set_xticks(names)
...
def visualizeTrends(coeff1973, coeff1983, coeff1995, coeff2005, coeff2013, coeff2017, df, benchmark, factor):
    ...
    df1973m = int(df1973[benchmark].mean())
    df1983m = int(df1983[benchmark].mean())
    df1995m = int(df1995[benchmark].mean())
    df2005m = int(df2005[benchmark].mean())
    df2013m = int(df2013[benchmark].mean())
    df2017m = int(df2017[benchmark].mean())
    ...
    for i in range(0, len(coeffs)):
        ...
        print(f'{years[i]} female decrease in percentage: {percentage}%')
        year = years[i]
        if percentage > 0:
            ylim = True
    ...
    fix, ax = plt.subplots()
    ax.set_title(f'Development of comparative income based on {factor}')
